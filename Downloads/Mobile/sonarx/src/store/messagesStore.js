...
export const useMessagesStore = create()((set) => ({
  ...
  addMessage: (chatId, message) =>
    set((state) => {
      ...
      return updateMessagesByChatId(state, chatId, () => [
        message,
        ...existing,
      ]);
    }),
  updateMessageStatus: (chatId, messageId, status, timestamp) =>
    set((state) =>
      updateMessagesByChatId(state, chatId, (msgs) =>
        msgs.map((m) =>
          m.id === messageId
            ? {
                ...m,
                status,
                readAt:
                  status === "read" ? (timestamp ?? Date.now()) : m.readAt,
              }
            : m,
        ),
      ),
  ...
  addOrUpdateChat: (chat) =>
    set((state) => {
      ...
      const updated =
        idx >= 0
          ? [...state.chats.slice(0, idx), chat, ...state.chats.slice(idx + 1)]
          : [chat, ...state.chats];
      return {
