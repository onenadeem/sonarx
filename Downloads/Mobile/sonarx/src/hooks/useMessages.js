...
import { getMessages, insertMessage, getOrCreateChat, updateChatLastMessage, updateMessageStatus as dbUpdateMessageStatus, } from '@/src/db/queries/messages';
...
export function useSendMessage(peerId) {
    ...
    const updateStoreStatus = useMessagesStore((state) => state.updateMessageStatus);
    const addOrUpdateChat = useMessagesStore((state) => state.addOrUpdateChat);
    const [isSending, setIsSending] = useState(false);
    const sendMessage = useCallback(async (text, replyToId) => {
        if (!identity)
            return;
        setIsSending(true);
        try {
