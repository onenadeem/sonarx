...
function SearchBar({ value, onChangeText, onClear, }) {
    ...
    return (<View style={styles.searchContainer}>
      <TextInput value={value} onChangeText={onChangeText} leftIcon="search-outline" rightIcon={value ? 'close-outline' : undefined} onRightIconPress={onClear} placeholder="Search contacts..." containerStyle={styles.searchInputContainer} inputWrapperStyle={[
...
function HighlightedText({ text, query, baseStyle, highlightStyle, }) {
    ...
    return (<Text style={baseStyle} numberOfLines={1}>
      {start}
...
function ContactRow({ contact, query, onPress, }) {
    ...
    return (<ListItem title={<HighlightedText text={contact.displayName} query={query} baseStyle={[
                styles.contactName,
                {
                    color: colors.textPrimary,
                    fontFamily: typography.fontFamily.semiBold,
                },
            ]} highlightStyle={{ color: colors.accent }}/>} subtitle={<View style={styles.subtitleRow}>
          <HighlightedText text={maskPhoneNumber(contact.phoneNumber)} query={query.slice(-4)} baseStyle={[
                styles.contactPhone,
                {
    ...
    return (<View style={[
      ...
      <View style={[
        ...
        {sections.length === 0 ? (<EmptyState />) : (<SectionList ref={listRef} sections={sections} keyExtractor={(item) => item.id} stickySectionHeadersEnabled renderSectionHeader={({ section }) => (<View style={[
              ...
              </View>)} renderItem={({ item }) => (<ContactRow contact={item} query={searchQuery} onPress={handleContactPress}/>)} showsVerticalScrollIndicator={false} contentContainerStyle={{
