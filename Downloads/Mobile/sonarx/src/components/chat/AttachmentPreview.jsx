...
function AnimatedBar({ index, color }) {
  ...
  useEffect(() => {
    ...
    height.value = withRepeat(
      withSequence(
        withTiming(toHeight, { duration: baseDuration }),
        withTiming(BASE_WAVE_HEIGHT, { duration: baseDuration }),
      ),
      -1,
      false,
    );
  ...
  return (
    <Animated.View
      style={[styles.waveBar, { backgroundColor: color }, animStyle]}
    />
  );
}
  ...
  return (
    <View style={styles.waveform}>
      {Array.from({ length: BAR_COUNT }).map((_, i) => (
        <AnimatedBar key={i} index={i} color={colors.accent} />
      ))}
    </View>
  );
}
...
export default function AttachmentPreview({ attachments, onRemove }) {
  ...
  useEffect(() => {
    translateY.value = withTiming(
      attachments.length > 0 ? EMPTY_ATTACHMENT_OPACITY : HIDE_OFFSET_Y,
      {
    ...
    >
      ...
      >
        {attachments.map((item) =>
          mapAttachmentRenderer(item, onRemove, item.type),
        )}
      </ScrollView>
    </Animated.View>
  );
