...
const getVariantStyle = (colors, variant) => {
  ...
  return {
    backgroundColor: config.backgroundColorKey
      ? colors[config.backgroundColorKey]
      : "transparent",
    borderColor: config.borderColorKey
      ? colors[config.borderColorKey]
      : config.borderColor,
    textColor: config.textColorKey
      ? colors[config.textColorKey]
      : config.textColor,
    iconColor: config.iconColorKey
      ? colors[config.iconColorKey]
      : config.textColor,
  };
};
...
export default function Button({
  ...
  const resolvedRadius = rounded
    ? borderRadius.full
    : (RESOLVED_RADIUS[size] ?? RESOLVED_RADIUS.default);
        ...
        {isLoadingResolved ? (
          ...
          <View
            style={{
              ...
              flex: contentJustify === "space-between" ? 1 : undefined,
            }}
          >
