...
export default function OnlineBadge({ isOnline, size = 10, style, }) {
    ...
    useEffect(() => {
        if (isOnline) {
            opacity.value = withRepeat(withSequence(withTiming(ONLINE_BLINK_MIN, { duration: ONLINE_BLINK_DURATION_MS }), withTiming(ONLINE_BLINK_MAX, { duration: ONLINE_BLINK_DURATION_MS })), -1, false);
        }
        else {
            opacity.value = 1;
        }
