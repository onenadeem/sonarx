...
import type { ContactQRData } from "./ShareContactQR";
import type { PeerPresence } from "@/lib/p2p/discovery";

interface ScanContactQRProps {
  isOpen: boolean;
  onClose: () => void;
  onContactFound: (peerId: string, presence: PeerPresence) => void;
}

function ScanContactQR({ isOpen, onClose, onContactFound }: ScanContactQRProps) {
  ...
  const handleBarCodeScanned = ({ data }: { data: string }) => {
    ...
    try {
      const contactData: ContactQRData = JSON.parse(data);
    ...
    >
      <View className="flex-1 bg-black">
        ...
        <View className="absolute bottom-0 left-0 right-0 pb-8 px-6">
          {error && (
            <View className="bg-destructive/90 p-3 rounded-lg mb-4">
              <Text className="text-white text-center">{error}</Text>
            </View>
          )}
          
          {scanned && (
            <View className="bg-primary/90 p-3 rounded-lg mb-4">
