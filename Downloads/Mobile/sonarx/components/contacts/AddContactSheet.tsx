...
interface AddContactSheetProps {
  isOpen: boolean;
  onClose: () => void;
  onAddContact: (peerId: string, presence: PeerPresence) => void;
}
...
function AddContactSheet({
  ...
  const [foundPeer, setFoundPeer] = useState<PeerPresence | null>(null);
  ...
  const handleQRContactFound = (peerId: string, presence: PeerPresence) => {
  ...
  return (
    <>
      <View
        ...
        style={{ backgroundColor: "rgba(3,7,12,0.65)" }}
      >
        <View className="bg-card rounded-t-3xl p-6 border border-border">
          ...
          <View className="flex-row mb-4 bg-muted rounded-lg p-1">
            <TouchableOpacity
              ...
              className={`flex-1 py-2 rounded-md ${
                mode === "phone" ? "bg-background border border-border" : ""
              }`}
            >
              <Text
                className={`text-center ${mode === "phone" ? "font-semibold" : ""}`}
              >
                Phone Number
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setMode("qr")}
              className={`flex-1 py-2 rounded-md ${
                mode === "qr" ? "bg-background border border-border" : ""
              }`}
            >
              <Text
                className={`text-center ${mode === "qr" ? "font-semibold" : ""}`}
              >
                QR Code
              </Text>
            </TouchableOpacity>
          ...
          {mode === "phone" ? (
            <>
              <TextInput
                className="border border-input rounded-xl px-4 py-3 text-foreground bg-card"
            ...
            <View className="space-y-3">
              <TouchableOpacity
                ...
                className="bg-card rounded-lg p-4 flex-row items-center border border-border"
              >
                <Ionicons
                  name="share-outline"
                  size={20}
      ...
      <ScanContactQR
        ...
        onContactFound={handleQRContactFound}
      />
    </>
  );
}
