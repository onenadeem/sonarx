...
function MessageInput({ onSend, onAttachment, onVoiceRecord, placeholder = "Type a message...", className, }) {
    ...
    return (<View className={cn("flex-row items-end px-4 py-3 bg-background border-t border-border", className)}>
      {/* Attachment Button */}
      <TouchableOpacity onPress={onAttachment} className="p-2 mr-2 rounded-full bg-muted border border-border">
        <Ionicons name="attach-outline" size={18} color={Colors[colorScheme].text}/>
      </TouchableOpacity>

      {/* Text Input */}
      <View className="flex-1 bg-card rounded-3xl px-4 py-2.5 max-h-32 border border-border">
        <TextInput value={text} onChangeText={setText} placeholder={placeholder} placeholderTextColor="hsl(var(--muted-foreground))" multiline maxLength={2000} className="text-foreground" style={{ maxHeight: 100 }}/>
      </View>
