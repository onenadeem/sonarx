...
function FixedSectionHeader({
  ...
  subtitle,
  accessory,
}: FixedSectionHeaderProps) {
  return (
    <View className="px-4 py-2 border-b border-border/60 bg-background/95">
      <View className="flex-row items-center justify-between gap-3">
        <View className="flex-1 flex-row items-center">
          <View className="mr-3 h-7 w-1 rounded-full bg-primary/70" />
          <View className="flex-1">
          {label ? (
            <Text className="text-[10px] font-semibold tracking-[0.2em] text-muted-foreground uppercase">
              {label}
            </Text>
          ) : null}
...
export type { FixedSectionHeaderProps };
