...
export async function compressImage(uri, maxWidth = 1920, quality = 0.8) {
    ...
    const compressed = await manipulateAsync(uri, [{ resize: { width: newWidth, height: newHeight } }], { compress: compress, format: SaveFormat.JPEG });
...
export async function generateBlurHash(uri) {
    try {
        const small = await manipulateAsync(uri, [{ resize: { width: 32, height: 32 } }], { format: SaveFormat.PNG });
...
export async function createThumbnail(uri, size = 300) {
    const thumbnail = await manipulateAsync(uri, [{ resize: { width: size, height: size } }], { format: SaveFormat.JPEG, compress: 0.6 });
