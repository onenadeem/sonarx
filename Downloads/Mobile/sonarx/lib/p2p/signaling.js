...
const signalPath = (targetPeerId, sourcePeerId, type) =>
  `${SL_NAMESPACE}/signal/${targetPeerId}/${sourcePeerId}/${type}`;
const icePath = (targetPeerId, sourcePeerId) =>
...
function publishSignalingPayload(
  targetPeerId,
  sourcePeerId,
  payloadType,
  payload,
...
async function publishIceCandidatePayload(
  path,
  candidate,
  sourcePeerId,
  targetPublicKey,
...
function handleEncryptedSignal(
  data,
  mySecretKey,
  senderPublicKey,
  onParsed,
...
export async function sendOffer(
  ...
  await publishSignalingPayload(
    targetPeerId,
    myIdentity.phoneNumber,
    SIGNAL_TYPES.offer,
    offer,
    targetPublicKey,
    mySecretKey,
    SIGNAL_TTL_SECONDS.offer,
  );
}
...
export function subscribeToAnswer(
  ...
  return subscribeToGun(path, (data) => {
    handleEncryptedSignal(
      data,
      mySecretKey,
      senderPublicKey,
      onAnswer,
      "Failed to parse answer SDP",
    );
  });
}
export async function sendIceCandidate(
  ...
  await publishIceCandidatePayload(
    path,
    candidate,
    myIdentity.phoneNumber,
    targetPublicKey,
    mySecretKey,
    SIGNAL_TTL_SECONDS.ice,
  );
}
